...
export async function applyPromoCodeAction(rawCode: string, country = "US"): Promise<ApplyPromoResult> {
  ...
  const outcome = await validatePromoForCheckout(rawCode, session.user.id, cart.subtotal, country);
