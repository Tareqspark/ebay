...
This document defines **Cartebay** as a product: who it's for, what it solves, how it's built, and where it's going. `CLAUDE.md` is the day-to-day engineering reference; this file is the north star those engineering decisions should trace back to.
...
Cartebay is the middle: **a single vendor, one quality bar, one shipping and returns policy — with marketplace-scale selection and marketplace-grade browsing.** Every item is sold and fulfilled by Cartebay directly. The bet is that shoppers will trade a bit of marketplace-style bottom-dollar pricing for consistent trust, and that a deep, well-organized taxonomy (not just a search box) is still how a lot of people actually like to shop.
...
2. **Shallow single-vendor taxonomies.** Most single-vendor sites top out at 2 levels of navigation and dump everything else into a flat "filter by tag" grid. Cartebay's taxonomy is 3 levels deep and data-driven (1,652 category nodes) specifically so that mega menu, sidebar, breadcrumb, and category pages can all stay in sync automatically as the catalog grows.
...
- **Not a marketplace.** No third-party sellers, no seller ratings separate from Cartebay's own, no "sold by X, fulfilled by Y" ambiguity.
...
Phase numbering here matches the client-facing proposal (`Cartebay-Proposed-Feature-List.pdf`) — five phases, grouped by feature area rather than by technical layer. `FEATURES.md` is the engineering-detail expansion of the same five phases.
