...
**Current status:** hybrid — the catalog, taxonomy, navigation, search, and merchandising surfaces are implemented against generated static data, same as before. But accounts, cart, checkout, and reviews are now real and backed by a real MySQL database (see *Tech stack* and `CLAUDE.md`): sign-up/sign-in, a persistent server-side cart (guest + logged-in, merged on login), a full checkout flow with Stripe payments, order-confirmation emails via SendGrid, real order history, an address book, and customer-submitted product reviews. The admin console (`/admin`) — including the hybrid self-stocked + CJdropshipping sourcing model — still runs entirely against generated mock data; it is not yet wired to the real database, and there's no real CJdropshipping API connection. Two things gate the new commerce flow going fully live: `STRIPE_SECRET_KEY`/`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` and `SENDGRID_API_KEY` are unset in `.env.local` — until they're filled in, checkout stops with a clear "not configured" message instead of charging a card, and confirmation emails log to the server console instead of sending.
...
Two roles exist functionally today; the rest are designed-for, to keep the data model and route structure from having to be reshaped later:
...
app/
  ...
  admin/                 the back-office console (dashboard, products, orders, inventory, supplier/CJ, settings, etc.) — still generated mock data, not yet wired to the real database
...
- **Phase 2 — Accounts, Cart & Checkout** *(done)*: real MySQL-backed accounts (Auth.js, bcrypt, JWT sessions), persistent server-side cart with guest-to-account merge, address book, order history, Stripe-powered checkout (Payment Element, webhook-driven idempotent order creation), and SendGrid order-confirmation email. Both Stripe and SendGrid degrade gracefully to a clear "not configured" state without live keys — see `CLAUDE.md`. Remaining: activating production Stripe/SendGrid credentials (no further development work); inventory decrement on order (self-stocked items should reduce `inventory.available`, CJ items shouldn't since CJ holds the stock — not yet wired since `/admin` inventory is still generated mock data, tracked under Phase 5.2); the source-aware mixed-order shipping-time messaging decision flagged in `FEATURES.md` is still open.
- **Phase 3 — Operations Console**: *UI built* — product/order/inventory/customer/marketing/content management. Remaining: wire it to the real database that now powers the storefront (tracked under Phase 5.2 — real storefront orders don't appear in `/admin/orders` yet, that still reads generated mock data), add auth/role gating (tracked under Phase 5.3, see *User roles*), and replace the still-manual `scripts/*-source.mjs` editing for category/brand taxonomy and homepage rail configuration.
- **Phase 4 — Hybrid Supplier Sourcing**: *UI built* — the dedicated CJdropshipping module (`/admin/cj`: catalog browse/import, fulfillment queue, after-sales, sourcing requests, settings) runs against a generated 50,000-item catalog snapshot, not a live CJdropshipping API connection. Remaining: real-time supplier integration, tracked under Phase 5.1.
- **Phase 5 — Advanced Capabilities**: *not started*. Real-time CJdropshipping API integration (5.1), a unified database connecting the Operations Console to the same live data as the storefront (5.2), staff roles & permissions (5.3), a dedicated search engine plus deeper personalization and account-synced wishlist (5.4), promo codes/loyalty/bundle pricing (5.5), and returns/live carrier shipping/analytics (5.6). See `FEATURES.md` for the full breakdown.
