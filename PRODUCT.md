...
| Admin console | **Live** | Full back-office at `/admin`: Dashboard, Products, Orders, Inventory, Categories, Collections, Customers, Reviews, Marketing, Supplier, Payments, Analytics, Shipping, Content, Settings — polished data tables, slide-over detail panels, KPIs, all reading real MySQL. Gated by real staff authentication (`admin_users` table, `/admin/login`, enforced in `middleware.ts`) — see *User roles*. Order fulfillment/refund/CJ-push, review moderation, CJ dispute resolution, product edits/delete, inventory correction, and API key management all write for real and log to a real audit trail; collections/content/marketing/shipping-rate/team-invite screens are still read-only (no create/edit UI built yet). |
...
- **Phase 3 — Operations Console** *(done)*: every admin screen (product/order/inventory/customer/payments/reviews/CJ ops/marketing/content/shipping/team settings) reads live MySQL — the entire generated-mock-data layer (`app/data/admin/*.ts`) has been retired from the admin console. `/admin/*` is gated by real staff authentication (`admin_users` table, bcrypt, a dedicated `/admin/login`, checked in `middleware.ts` before any admin page renders) — customer accounts cannot reach it. Core day-to-day mutations write for real and log to a real audit trail (`activity_events`): order fulfillment/cancel/refund (refund calls the real Stripe refund API when configured)/push-to-CJ, review approve/reject, CJ dispute resolve/reject, product price/cost/status/visibility edits and delete, and manual inventory stock correction. Remaining: collections/content/marketing-campaign/shipping-rate/team-invite screens are still read-only lists (no create/edit UI exists yet — a bigger lift than the mutation-wiring above, since the UI itself isn't built, not just disconnected); category/brand taxonomy is still edited through `scripts/*-source.mjs`, not the admin Categories screen; staff `role` (Owner/Admin/Merchandiser/Support/Catalog Manager) is stored and displayed but not yet enforced — any active staff account can reach every admin screen regardless of role.
