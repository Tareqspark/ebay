...
| Supplier sourcing (CJdropshipping hybrid) | **Live (UI only)** | Products are either self-stocked (wholesale-restocked into Baruashop's own warehouses) or dropshipped via CJdropshipping — modeled per-product via a `source: "self" \| "cj"` field. A 50,000-item generated CJ catalog is server-side paginated/searchable at `/admin/supplier/cj-catalog` with bulk import; Orders carry independent self/CJ fulfillment tracks with a "Push to CJ" action; `/admin/supplier/after-sales` handles CJ claims (lost/damaged/wrong-item/defective); `/admin/settings/cj` holds the (mock) CJ account connection, wallet balance, and sourcing-request form. No real CJ API/webhook connection exists yet — see `CLAUDE.md` for the data-loading architecture. |
